...
- `/workspace/:projectId`：全画幅节点画布：左侧搜索式节点添加面板、最近使用节点、快速添加上游/下游节点并自动连线、节点面板拖拽添加、节点大纲搜索、问题筛选和批量选中、工作流预设、我的工作流预设、项目分镜面板和素材/任务入口，中间无限画布，右侧节点/连线/选区参数面板，支持按单个或全部分镜添加生成链路、空白处双击创建节点、拖入本地或 URL 素材、节点拖拽、框选/多选、全选/反选/清空选区、上游/下游链路选择、同类型/同状态/同标记/禁用/孤立/起点/终点/错误/提醒/运行中任务/失败任务节点选择、连线同起点/同终点/同标签/同颜色/同样式选择与批量编辑、选区对齐与等距分布、选区打组/选中同组节点/分组名称编辑/取消分组、选区内部连线批量启用/禁用、颜色标记、样式和删除、节点语义输入/输出连接桩连线、连线规则提示、节点右键快捷菜单、连线右键快捷菜单、选区批量操作、选区批量命名、连线标签编辑、连线颜色标记、连线样式、连线端口映射、连线插入节点、连线启用/禁用、连线同起点/同终点/同标签/同颜色/同样式选择与批量编辑、节点备注、节点颜色标记、节点折叠/展开、节点参数编辑可撤销、节点类型切换、节点锁定/解锁、节点启用/禁用、自动整理和选区整理、画布自检、运行前缺口提示、撤销/重做、素材预览、上游输入预览和参数填充、工作流 JSON 导入导出、自定义预设保存和复用、视口保存/恢复、网格吸附、适配全部节点、适配选区和重置视口、生成参数预设、保存、单节点运行、当前上游链路运行、选区运行、全图终点链路运行、节点复制、选区复制粘贴、选区保存为预设、链路复制粘贴、删除、分镜绑定、生成参数编辑、连线补全、节点运行诊断、任务定位、任务状态同步、重试和取消。
