...
- 已完成 LibTV 风格节点画布创作端：用户端与后台端分离，`/workspace/[projectId]` 进入全画幅节点画布，可搜索添加、拖拽、连线、保存和运行文本、图片、视频、音频、脚本 Beta、分镜图、镜头视频、TTS、合成和演示节点；节点图支持后端持久化与本地草稿兜底；生成节点已支持分镜下拉绑定、尺寸、seed、首帧、时长、fps、音色、语速、字幕合成等参数，并可从上游连线补全分镜、提示词、首帧图片和旁白文本；画布已内置脚本拆解分镜、首帧图生视频、旁白字幕合成三类工作流预设，可一键铺设成组节点和连线；画布可打开项目分镜面板，并为任意分镜一键添加文本、分镜图、视频、配音和合成节点链路；参数面板支持运行单节点、运行当前上游链路、复制节点和删除节点。
...
- `/workspace/:projectId`：全画幅节点画布：左侧搜索式节点添加面板、工作流预设、项目分镜面板和素材/任务入口，中间无限画布，右侧节点参数面板，支持按分镜添加生成链路、节点拖拽、连线、保存、单节点运行、当前上游链路运行、节点复制、删除、分镜绑定、生成参数编辑和连线补全。
...
- 第 3 阶段：创作工作台。完成脚本拆解、角色设定、分镜表格、单镜头重试、批量生成、素材预览、时间线编排、成片合成和导出。已追加完成 LibTV 风格全画幅节点画布，支持平台业务节点编排、项目分镜面板、按分镜添加生成链路、工作流预设、搜索添加、分镜绑定、参数编辑、连线补全、节点复制、单节点运行、链路运行和保存。
...
- 工作台流程：脚本输入后能生成分镜，单个分镜可重试，批量生成不会阻塞页面；全画幅节点画布可通过项目分镜面板、工作流预设或搜索面板添加节点，可按单个分镜铺设文本、分镜图、视频、配音和合成链路，可连线、保存、复制、删除、单节点运行和上游链路运行平台业务节点，生成节点可从分镜下拉或上游连线补全业务参数。
