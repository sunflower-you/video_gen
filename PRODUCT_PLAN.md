...
- 已完成 LibTV 风格节点画布创作端：用户端与后台端分离，`/workspace/[projectId]` 进入全画幅节点画布，可搜索添加、空白处双击创建、拖入本地或 URL 素材、拖拽、连线、自动整理、保存和运行文本、图片、视频、音频、脚本 Beta、分镜图、镜头视频、TTS、合成和演示节点；节点图支持后端持久化、本地草稿兜底、链路剪贴板、工作流 JSON 导入导出和本地自定义工作流预设；生成节点已支持分镜下拉绑定、尺寸、seed、首帧、时长、fps、音色、语速、字幕合成等参数，并可从上游连线补全分镜、提示词、首帧图片和旁白文本，参数面板也能预览上游输入并一键填充当前节点参数；画布已内置脚本拆解分镜、首帧图生视频、旁白字幕合成三类工作流预设，可一键铺设成组节点和连线，用户也可把当前画布保存为“我的工作流预设”后跨项目复用；画布可打开项目分镜面板，并为单个或全部分镜一键添加文本、分镜图、视频、配音和合成节点链路；节点卡片、参数面板和素材库可直接预览图片、视频、音频和生成输出素材；参数面板支持运行单节点、运行当前上游链路、运行全画布终点链路、复制节点、复制/粘贴上游链路和删除节点，并可查看选中节点关联任务的进度、错误、重试建议、事件日志和 ComfyUI prompt_id；任务队列支持状态统计、定位关联节点、提交、同步、重试和取消任务；画布自检可在运行前检查断线、孤立节点、缺少分镜、提示词、首帧、配音文本和合成输入等问题并定位到节点；画布支持保存、运行全图、整理、导入导出工作流、复制、粘贴、删除等键盘快捷操作。
...
- `/workspace/:projectId`：全画幅节点画布：左侧搜索式节点添加面板、工作流预设、我的工作流预设、项目分镜面板和素材/任务入口，中间无限画布，右侧节点参数面板，支持按单个或全部分镜添加生成链路、空白处双击创建节点、拖入本地或 URL 素材、节点拖拽、连线、自动整理、画布自检、素材预览、上游输入预览和参数填充、工作流 JSON 导入导出、自定义预设保存和复用、保存、单节点运行、当前上游链路运行、全图终点链路运行、节点复制、链路复制粘贴、删除、分镜绑定、生成参数编辑、连线补全、节点运行诊断、任务定位、任务状态同步、重试和取消。
...
- 第 3 阶段：创作工作台。完成脚本拆解、角色设定、分镜表格、单镜头重试、批量生成、素材预览、时间线编排、成片合成和导出。已追加完成 LibTV 风格全画幅节点画布，支持平台业务节点编排、项目分镜面板、按单个或全部分镜添加生成链路、工作流预设、我的工作流预设、搜索添加、空白处双击添加、素材拖入、画布内素材预览、上游输入预览和参数填充、分镜绑定、参数编辑、连线补全、节点复制、链路复制粘贴、工作流 JSON 导入导出、单节点运行、链路运行、全图运行、自动整理、画布自检、运行诊断、任务操作和保存。
...
- 工作台流程：脚本输入后能生成分镜，单个分镜可重试，批量生成不会阻塞页面；全画幅节点画布可通过项目分镜面板、工作流预设、我的工作流预设、搜索面板、空白处双击或拖入素材添加节点，可按单个分镜或全部分镜铺设文本、分镜图、视频、配音和合成链路，可连线、自动整理、导入导出工作流 JSON、保存自定义预设、保存画布、复制节点、复制粘贴上游链路、删除、画布自检、素材预览、上游输入预览和参数填充、单节点运行、上游链路运行和全画布终点链路运行平台业务节点，生成节点可从分镜下拉或上游连线补全业务参数，节点和任务队列能追踪任务状态、错误、重试建议、事件日志和 ComfyUI prompt_id，并能提交、同步、重试、取消任务。
