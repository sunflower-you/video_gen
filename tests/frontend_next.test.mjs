...
test("Next 首页呈现用户创作入口并隐藏后台能力", () => {
  ...
  assert.match(gallery, /window\.location\.href = await createSameStyleProjectFromHref/);
  assert.match(gallery, /正在创建\$\{title\}全画幅画布/);
...
test("Next 全屏创作画布支持节点编排和平台节点运行", () => {
  ...
  assert.match(canvasWorkspace, /resetCommandPaletteSearch/);
  assert.match(canvasWorkspace, /已清空命令搜索，可继续查找运行、导入导出、节点面板和快捷创作命令/);
  assert.match(canvasWorkspace, /清空搜索<\/button>/);
  assert.match(canvasWorkspace, /closeCommandPalette\(\); setShowPalette\(true\)/);
  ...
  assert.match(canvasWorkspace, /first_frame_url/);
  ...
  assert.match(canvasWorkspace, /当前筛选结果没有可提交任务，已打开任务队列；可先清空筛选、调整任务状态或运行画布生成新任务/);
  assert.match(canvasWorkspace, /当前筛选结果没有可同步任务，已打开任务队列；可先清空筛选或调整任务状态/);
  assert.match(canvasWorkspace, /当前筛选结果没有失败任务可重试，已打开任务队列；可先切换到失败任务或同步任务状态/);
  assert.match(canvasWorkspace, /当前筛选结果没有待生成或生成中的任务可取消，已打开任务队列；可先切换状态筛选或同步任务状态/);
  assert.match(canvasWorkspace, /没有匹配素材/);
  assert.match(canvasWorkspace, /没有匹配任务/);
  ...
  assert.match(canvasWorkspace, /reference_image_url/);
  assert.match(canvasWorkspace, /model_key/);
  assert.match(canvasWorkspace, /style_prompt/);
  ...
  assert.match(canvasWorkspace, /请先在项目分镜面板选择一个或多个分镜；已打开分镜面板，可先筛选并选择要绑定的分镜/);
  assert.match(canvasWorkspace, /请先框选文本、脚本、分镜图、视频或配音节点，再绑定分镜；已打开节点大纲，可先定位可绑定节点/);
  assert.match(canvasWorkspace, /已把 .*个分镜绑定到选区 .*个节点/);
  assert.match(canvasWorkspace, /选中同分镜节点/);
  assert.match(canvasWorkspace, /当前节点没有绑定分镜，无法选中同分镜节点；已打开分镜面板，可先为节点绑定分镜/);
  assert.match(canvasWorkspace, /同分镜/);
  assert.match(canvasWorkspace, /同步同分镜参数/);
  assert.match(canvasWorkspace, /请先选择一个绑定分镜的节点，再同步同分镜参数；已打开节点大纲，可先定位已绑定分镜的源节点/);
  assert.match(canvasWorkspace, /当前节点没有绑定分镜，无法同步同分镜参数；已打开分镜面板，可先为源节点绑定分镜/);
  assert.match(canvasWorkspace, /当前节点没有可同步的业务参数，已打开节点大纲；可先选择带提示词、媒体、生成参数或配音参数的源节点/);
  assert.match(canvasWorkspace, /同分镜下没有其它同类型节点可同步参数，已打开分镜面板；可先为该分镜追加同类型生成节点/);
  assert.match(canvasWorkspace, /同步参数/);
  assert.match(canvasWorkspace, /应用当前参数到选区/);
  assert.match(canvasWorkspace, /请先选择一个源节点和至少一个同类型目标节点，再应用当前参数到选区；已打开节点大纲，可先定位并多选同类型节点/);
  assert.match(canvasWorkspace, /当前节点没有可应用到选区的业务参数，已打开节点大纲；可先选择带提示词、媒体、生成参数或配音参数的源节点/);
  assert.match(canvasWorkspace, /当前选区没有其它同类型节点可应用参数，已打开节点大纲；可先多选同类型目标节点后重试/);
  ...
  assert.match(canvasWorkspace, /模型 Key/);
  assert.match(canvasWorkspace, /统一风格提示词/);
  ...
  assert.match(canvasWorkspace, /resetShotFilters/);
  assert.match(canvasWorkspace, /addAllProjectShotWorkflows/);
  assert.match(canvasWorkspace, /暂无分镜，可先追加脚本拆解工作流、打开节点面板手动搭建，或导入已有 ProjectGraph 工作流继续创作/);
  assert.match(canvasWorkspace, /没有匹配分镜，可清空筛选、铺设全部分镜链路，或只选择当前未铺设分镜继续整理/);
  assert.match(canvasWorkspace, /清空分镜筛选<\/button>/);
  ...
  assert.match(canvasWorkspace, /工作流 JSON 中没有节点，已打开导入工作流面板；可先导出当前画布、选择有效文件，或追加内置工作流预设/);
  ...
  assert.match(canvasWorkspace, /节点已禁用，已打开节点大纲并定位当前节点；请先启用节点后再运行/);
  ...
  assert.match(canvasWorkspace, /当前链路没有可运行节点，已跳过禁用节点、批注或角色参考节点；已打开节点大纲，可先启用链路中的生成节点或选择其它链路/);
  assert.match(canvasWorkspace, /选区没有可运行节点，已跳过禁用节点、批注或角色参考节点；已打开节点大纲，可先选择文本、图片、视频、配音或合成生成节点/);
  assert.match(canvasWorkspace, /画布暂无可运行节点，已打开节点面板；请先添加节点或工作流/);
  assert.match(canvasWorkspace, /画布没有可运行节点，已跳过禁用节点、批注或角色参考节点；已打开节点面板，可先添加或启用生成节点/);
  ...
  assert.match(canvasWorkspace, /TV Show 棚拍/);
  assert.match(canvasWorkspace, /image-challenge-cover/);
  assert.match(canvasWorkspace, /挑战赛封面/);
  assert.match(canvasWorkspace, /video-seedance-camera/);
  assert.match(canvasWorkspace, /Seedance 同款运镜/);
  assert.match(canvasWorkspace, /video-challenge-push/);
  assert.match(canvasWorkspace, /挑战赛快速推进/);
  assert.match(canvasWorkspace, /video-tv-show-pan/);
  assert.match(canvasWorkspace, /TV Show 横移开场/);
  assert.match(canvasWorkspace, /tts-tv-host/);
  assert.match(canvasWorkspace, /TV Show 主持/);
  assert.match(canvasWorkspace, /tts-challenge-drama/);
  assert.match(canvasWorkspace, /挑战赛悬念口播/);
  assert.match(canvasWorkspace, /compose-tv-show-package/);
  assert.match(canvasWorkspace, /TV Show 包装/);
  assert.match(canvasWorkspace, /compose-challenge-package/);
  assert.match(canvasWorkspace, /挑战赛成片包装/);
  ...
  assert.match(canvasWorkspace, /请先框选或点选节点，再复制选区；已打开节点大纲，可先定位并选择要复用的节点/);
  assert.match(canvasWorkspace, /请先框选或点选节点，再剪切选区；已打开节点大纲，可先定位并选择要移动的节点/);
  assert.match(canvasWorkspace, /请先选择一个节点，再复制链路；已打开节点大纲，可先定位链路终点节点/);
  ...
  assert.match(canvasWorkspace, /character: "角色节点"/);
  assert.match(canvasWorkspace, /项目角色库/);
  assert.match(canvasWorkspace, /角色一致性参考/);
  assert.match(canvasWorkspace, /characterOptions/);
  assert.match(canvasWorkspace, /filteredCharacters/);
  assert.match(canvasWorkspace, /emptyCharacterDraft/);
  assert.match(canvasWorkspace, /startNewCharacterDraft/);
  ...
  assert.match(canvasWorkspace, /新建项目角色/);
  assert.match(canvasWorkspace, /编辑项目角色/);
  ...
  assert.match(canvasWorkspace, /角色参考图/);
  assert.match(canvasWorkspace, /角色风格/);
  assert.match(canvasWorkspace, /isReferenceOnlyNode/);
  ...
  assert.match(canvasWorkspace, /type === "comment" \|\| type === "character"\) continue/);
  assert.match(canvasWorkspace, /禁用节点、批注或角色参考节点/);
  ...
  assert.match(canvasWorkspace, /ArrowDown/);
  assert.match(canvasWorkspace, /ArrowUp/);
  assert.match(canvasWorkspace, /当前节点面板没有匹配节点，已保持节点面板打开；可先清空筛选、切换分类或换关键词/);
  assert.match(canvasWorkspace, /已定位节点面板项/);
  assert.match(canvasWorkspace, /当前节点面板没有匹配节点，已保持节点面板打开；可先清空筛选、切换分类或换关键词后再快速添加/);
  ...
  assert.match(canvasWorkspace, /resetNodePaletteFilters/);
  assert.match(canvasWorkspace, /已清空节点面板筛选，可继续添加文本、图片、视频、配音和生成节点/);
  assert.match(canvasWorkspace, /清空筛选<\/button>/);
  assert.match(canvasWorkspace, /追加脚本拆解<\/button>/);
  assert.match(canvasWorkspace, /新建 Seedance<\/button>/);
  ...
  assert.match(canvasWorkspace, /Seedance<\/button>/);
  assert.match(canvasWorkspace, /TV Show<\/button>/);
  assert.match(canvasWorkspace, /挑战赛<\/button>/);
  assert.match(canvasWorkspace, /replaceCanvasWithWorkflowPreset/);
  assert.match(canvasWorkspace, /新建 Seedance<\/button>/);
  assert.match(canvasWorkspace, /新建 TV Show<\/button>/);
  assert.match(canvasWorkspace, /新建挑战赛<\/button>/);
  assert.match(canvasWorkspace, /已替换当前画布为快捷创作预设/);
  ...
  assert.match(canvasWorkspace, /replace-seedance-preset/);
  assert.match(canvasWorkspace, /replace-tv-show-preset/);
  assert.match(canvasWorkspace, /replace-creator-challenge-preset/);
  ...
  assert.match(canvasWorkspace, /追加创作者挑战赛参赛片链路/);
  ...
  assert.match(canvasWorkspace, /追加 TV Show 剧集开场链路/);
  assert.match(canvasWorkspace, /TV Show 剧集脚本/);
  assert.match(canvasWorkspace, /TV Show 成片合成/);
  ...
  assert.match(canvasWorkspace, /追加 Seedance 2\.0 图生视频链路/);
  ...
  assert.match(canvasWorkspace, /params\.get\("quickScript"\)/);
  ...
  assert.match(canvasWorkspace, /params\.get\("sourceDuration"\)/);
  assert.match(canvasWorkspace, /params\.get\("sourceFps"\)/);
  assert.match(canvasWorkspace, /params\.get\("sourceVoice"\)/);
  assert.match(canvasWorkspace, /params\.get\("sourceRate"\)/);
  ...
  assert.match(canvasWorkspace, /params\.delete\("quickScript"\)/);
  ...
  assert.match(canvasWorkspace, /params\.delete\("sourceDuration"\)/);
  assert.match(canvasWorkspace, /params\.delete\("sourceFps"\)/);
  assert.match(canvasWorkspace, /params\.delete\("sourceVoice"\)/);
  assert.match(canvasWorkspace, /params\.delete\("sourceRate"\)/);
  ...
  assert.match(canvasWorkspace, /generationParamOverride/);
  assert.match(canvasWorkspace, /duration: overrides\.sourceDuration/);
  assert.match(canvasWorkspace, /fps: overrides\.sourceFps/);
  assert.match(canvasWorkspace, /voice: overrides\.sourceVoice/);
  assert.match(canvasWorkspace, /rate: overrides\.sourceRate/);
...
test("Next 模板市场读取真实模板并支持复刻项目", () => {
  ...
  assert.match(templates, /function syncTemplateFiltersFromLocation/);
  assert.match(templates, /window\.location\.pathname !== "\/templates"/);
  assert.match(templates, /params\.get\("channel"\) \|\| "全部"/);
  assert.match(templates, /params\.get\("keyword"\) \|\| ""/);
  assert.match(templates, /function writeTemplateFilters\(nextChannel: string, nextKeyword: string\)/);
  assert.match(templates, /params\.set\("channel", nextChannel\)/);
  assert.match(templates, /params\.set\("keyword", nextKeyword\.trim\(\)\)/);
  assert.match(templates, /window\.history\.replaceState\(null, "", nextUrl\)/);
  ...
  assert.match(fallbackData, /quick\/tv_show_storyboard/);
  assert.match(fallbackData, /TV Show 剧集开场/);
  assert.match(fallbackData, /workflow_key: "tv_show_storyboard"/);
  assert.match(templateQuickStart, /\/create\?quick=creator-challenge/);
  assert.match(templateQuickStart, /\/create\?quick=tv-show/);
  ...
  assert.match(templateQuickStart, /sourceDuration/);
  assert.match(templateQuickStart, /sourceFps/);
  assert.match(templateQuickStart, /sourceVoice/);
  assert.match(templateQuickStart, /sourceRate/);
  ...
  for (const text of ["复刻项目", "快速同款创作", "分享模板", "分享定位", "搜索模板", "搜索模板、工作流、适用场景", "清空", "当前筛选", "复刻项目标题", "目标画幅", "9:16 竖屏短视频", "16:9 横屏短片", "1:1 方形画布", "刷新模板", "模板复刻成功", "模板复刻失败，请稍后重试", "默认参数", "示例输入", "使用次数", "查看封面", "查看成片示例", "全部", "创作者挑战赛", "Seedance 2.0", "TV Show", "创作", "开始创作"]) {
...
test("Next 创作工作台调用真实项目和生成接口", () => {
  ...
  assert.match(createWorkbench, /activeQuickMode === "tv-show"/);
  assert.match(createWorkbench, /activeQuickMode === "creator-challenge"/);
  ...
  assert.match(createWorkbench, /\{ referenceImageUrl, quickScript: script, \.\.\.sameStyleParams\(\) \}/);
  assert.match(createWorkbench, /\{ quickScript: script, \.\.\.sameStyleParams\(\) \}/);
...
test("Next 作品详情页接入真实作品接口和互动入口", () => {
  ...
  assert.match(workDetail, /href=\{`\/\?category=\$\{encodeURIComponent\(work\?\.category \|\| "全部"\)\}`\}/);
  ...
  assert.match(workDetail, /href="\/templates">查看模板市场/);
  ...
  for (const text of ["作品详情", "同款创作", "点赞", "收藏", "分享", "查看作者主页", "模板：", "未绑定模板", "相似工作流", "继续同款创作", "暂无标签", "浏览同类作品", "作品数据", "成片预览"]) {
...
test("Next 作者主页读取真实作者聚合并支持关注", () => {
  ...
  assert.match(authorProfile, /function copyTemplateShareLink/);
  ...
  assert.match(authorProfile, /function templateChannelHref\(template: AuthorProfile\["templates"\]\[number\]\)/);
  assert.match(authorProfile, /\/templates\?template=\$\{encodeURIComponent\(template\.id\)\}/);
  assert.match(authorProfile, /\/templates\?channel=\$\{encodeURIComponent\(template\.category \|\| "全部"\)\}/);
  assert.match(authorProfile, /href=\{templateMarketHref\(template\)\}/);
  assert.match(authorProfile, /href=\{templateChannelHref\(template\)\}/);
  ...
  assert.match(authorProfile, /template_share_link_\$\{template\.id\}/);
  assert.match(authorProfile, /已复制「\$\{profile\.nickname\}」主页链接/);
  assert.match(authorProfile, /已复制《\$\{work\.title\}》分享链接/);
  assert.match(authorProfile, /已复制「\$\{template\.name\}」模板分享链接/);
