...
export function quickStartHrefForWork(item: Pick<Work, "id" | "title" | "category" | "cover_url" | "template_name" | "template_id" | "tags"> | null | undefined): string {
  if (!item) return "/create";
  const text = `${item.category} ${item.template_name || ""} ${item.template_id || ""} ${(item.tags || []).join(" ")}`;
  const sourceScript = [item.title, item.tags?.join("、")].filter(Boolean).join("：");
  const source = { sourceTitle: item.title, sourceWorkId: item.id, sourceTemplateId: item.template_id, sourceScript, sourceReferenceUrl: item.cover_url };
