...
const sameShotSyncKeysByType: Record<string, string[]> = {
  ...
  compose_generation: ["workflow_key", "duration_per_shot", "subtitle_style", "transition", "subtitle", "voice", "bgm_url"]
};
...
function shotPatchForNode(type: string, shot: StoryboardShot) {
  ...
  if (type === "image_generation") {
    patch.prompt = shot.prompt || visual;
    patch.negative_prompt = shot.negative_prompt || "";
...
const workflowPresets = [
  {
    ...
    nodes: [
      ...
      { type: "compose_generation", offset: { x: 620, y: 40 }, data: { title: "成片合成", workflow_key: "", duration_per_shot: "4", subtitle_style: "底部白字黑描边", transition: "cut", subtitle: true, voice: "zh-CN-XiaoxiaoNeural", bgm_url: "" } }
    ],
  ...
  },
  {
    ...
    nodes: [
      ...
      { type: "compose_generation", offset: { x: 620, y: 0 }, data: { title: "成片合成", workflow_key: "", duration_per_shot: "4", subtitle_style: "底部白字黑描边", transition: "cut", subtitle: true, voice: "zh-CN-XiaoxiaoNeural", bgm_url: "" } }
    ],
    edges: [[0, 1], [1, 2]]
  },
  {
  ...
  },
  {
    ...
    nodes: [
      ...
      { type: "compose_generation", offset: { x: 610, y: 0 }, data: { title: "成片合成", workflow_key: "", duration_per_shot: "4", subtitle_style: "底部白字黑描边", transition: "cut", subtitle: true, voice: "zh-CN-XiaoxiaoNeural", bgm_url: "" } }
    ],
    edges: [[0, 1], [1, 2]]
  }
];
...
export function CanvasWorkspace({ projectId }: { projectId: string }) {
                ...
                : type === "video_generation"
                  ...
                  : type === "tts_generation"
                    ...
                    : type === "compose_generation"
                      ? { title: nodeLabels[type], duration_per_shot: "4", subtitle_style: "底部白字黑描边", transition: "cut", subtitle: true, voice: "zh-CN-XiaoxiaoNeural", bgm_url: "" }
  ...
  function buildShotWorkflow(shot: StoryboardShot, timestamp: number, baseX: number, baseY: number) {
    const specs = [
      ...
      { type: "compose_generation", offset: { x: 920, y: 30 }, data: { title: `分镜 ${shot.index} 合成`, workflow_key: "", duration_per_shot: "4", subtitle_style: "底部白字黑描边", transition: "cut", subtitle: true, voice: "zh-CN-XiaoxiaoNeural", bgm_url: "" } }
    ];
  ...
  async function batchFilteredTaskAction(action: "submit" | "sync" | "retry" | "cancel") {
    ...
    setBusy(true);
    const actionLabel = action === "submit" ? "提交" : action === "sync" ? "同步" : action === "retry" ? "重试" : "取消";
    setStatus(`正在批量${actionLabel} ${targets.length} 个筛选任务...`);
    let successCount = 0;
    let failedCount = 0;
    for (const taskItem of targets) {
      const path = action === "sync" ? `/api/comfy/tasks/${taskItem.id}/sync` : `/api/tasks/${taskItem.id}/${action}`;
      try {
        const task = await postJson<GenerationTask>(path, {
          user_id: currentUserId(),
    ...
    setStatus(failedCount ? `批量${actionLabel}完成：成功 ${successCount} 个，失败 ${failedCount} 个。` : `已批量${actionLabel} ${successCount} 个任务。`);
    try {
      await refreshAll();
    } finally {
      setBusy(false);
  ...
  function collectSelectedNodesToCompose() {
    ...
    const composeNode = createFlowNode("compose_generation", { x: right + 360, y: (top + bottom) / 2 }, { title: "选区成片合成", duration_per_shot: "4", subtitle_style: "底部白字黑描边", transition: "cut", subtitle: true, voice: "zh-CN-XiaoxiaoNeural", bgm_url: "" });
    const timestamp = Date.now();
  ...
  function expandSelectedNodesToTtsGeneration() {
    ...
    const sourceNodes = terminalSelectedNodes.length ? terminalSelectedNodes : selectedNodes;
    const timestamp = Date.now();
    const ttsNodes = sourceNodes.map((node, index) => {
      const sourceData = node.data as Record<string, unknown>;
      const sourceTitle = String(sourceData.title || nodeLabels[String(sourceData.nodeType || "")] || "选区节点");
      const text = firstNonEmpty([sourceData], "narration", "text", "script", "prompt");
    ...
    const nextEdges = sourceNodes.flatMap((node, index) => {
      const connection = { source: node.id, target: ttsNodes[index].id, sourceHandle: "output", targetHandle: "input" };
      if (connectionIssueMessage(connection, edges)) return [];
      return [edgeWithDefaultHandles({
        id: `edge-selection-tts-${timestamp}-${index}`,
        ...connection,
        animated: true,
        data: { label: "文本生成配音" }
      } satisfies Edge)];
    });
    if (!ttsNodes.length || !nextEdges.length) {
      setStatus("当前选区没有可接入旁白配音的末端节点。");
      return;
    }
    rememberGraphHistory();
    setNodes((items) => [...items.map((node) => ({ ...node, selected: false })), ...ttsNodes.map((node) => ({ ...node, selected: true }))]);
    setEdges((items) => [...items, ...nextEdges]);
    setSelectedNodeId(ttsNodes[ttsNodes.length - 1]?.id || "");
    setSelectedEdgeId("");
    rememberRecentNodeType("tts_generation");
    setStatus(`已为选区末端创建 ${ttsNodes.length} 个旁白配音节点。`);
  }
  ...
  const commandPaletteItems: { key: string; title: string; description: string; shortcut?: string; disabled?: boolean; run: () => void }[] = [
    ...
    { key: "submit-filtered-tasks", title: "批量提交筛选任务", description: "按任务队列当前筛选条件提交任务到 ComfyUI", disabled: busy || !filteredTasks.length, run: () => void batchFilteredTaskAction("submit") },
    ...
    { key: "cancel-filtered-active-tasks", title: "批量取消筛选任务", description: "只取消当前筛选结果中的待生成或生成中任务", disabled: busy || !filteredTasks.some((task) => task.status === "pending" || task.status === "running"), run: () => void batchFilteredTaskAction("cancel") },
    ...
    <main className="h-screen overflow-hidden bg-[#0b1020] text-white">
      ...
      {showViewBookmarks && <aside className="absolute left-20 top-28 z-30 w-[340px] rounded-lg border border-white/10 bg-slate-950/95 p-4 shadow-2xl backdrop-blur">
        ...
        <div className="mt-3 grid gap-2 text-sm">
          ...
          {!viewBookmarks.length && <div className="rounded-md border border-white/10 px-3 py-2 text-slate-400">
            <p>暂无视图书签，可先保存当前视图。</p>
            <div className="mt-3 flex flex-wrap gap-2">
              <button className="rounded-md border border-blue-400/30 bg-blue-500/10 px-3 py-1 text-xs text-blue-50 hover:bg-blue-500/20" onClick={saveCurrentViewBookmark}>保存当前视图</button>
              <button className="rounded-md border border-white/10 px-3 py-1 text-xs text-slate-100 hover:bg-white/10" onClick={() => void copyCurrentViewLink()}>复制视图链接</button>
              <button disabled={!nodes.length} className="rounded-md border border-white/10 px-3 py-1 text-xs text-slate-100 hover:bg-white/10 disabled:opacity-50" onClick={fitGraphView}>适配全部节点</button>
            </div>
          </div>}
        </div>
      </aside>}
      ...
      {showGraphVersions && <aside className="absolute left-20 top-28 z-30 max-h-[620px] w-[380px] overflow-auto rounded-lg border border-white/10 bg-slate-950/95 p-4 shadow-2xl backdrop-blur">
        ...
        <div className="mt-3 grid gap-2 text-sm">
          ...
          {!graphVersions.length && <div className="rounded-md border border-white/10 px-3 py-2 text-slate-400">
            <p>暂无画布版本快照，可先保存当前版本。</p>
            <div className="mt-3 flex flex-wrap gap-2">
              <button disabled={!nodes.length} className="rounded-md border border-blue-400/30 bg-blue-500/10 px-3 py-1 text-xs text-blue-50 hover:bg-blue-500/20 disabled:opacity-50" onClick={saveCurrentGraphVersion}>保存当前版本</button>
              <button className="rounded-md border border-white/10 px-3 py-1 text-xs text-slate-100 hover:bg-white/10" onClick={() => void importGraphVersionFromClipboard()}>导入版本快照</button>
      ...
      <section className="absolute right-4 top-28 z-20 w-[360px] rounded-lg border border-white/10 bg-slate-950/90 p-4 shadow-2xl backdrop-blur">
        ...
        {selectedNodes.length > 1 ? <div className="mt-4 grid gap-3 text-sm">
          ...
          {selectedType === "image_generation" && <div className="grid grid-cols-2 gap-2">
            <label className="grid gap-1"><span className="text-slate-400">模型 Key</span><input className="rounded-md border border-white/10 bg-white/5 px-3 py-2 outline-none" placeholder="例如 flux-dev 或自定义模型标识" onFocus={rememberSelectedNodeEdit} value={String(selectedData.model_key || "")} onChange={(event) => updateSelectedData("model_key", event.target.value)} /></label>
            <label className="grid gap-1"><span className="text-slate-400">统一风格提示词</span><input className="rounded-md border border-white/10 bg-white/5 px-3 py-2 outline-none" placeholder="例如国漫线稿、电影感冷色调" onFocus={rememberSelectedNodeEdit} value={String(selectedData.style_prompt || "")} onChange={(event) => updateSelectedData("style_prompt", event.target.value)} /></label>
          </div>}
          ...
          {selectedType === "image_generation" && <div className="grid grid-cols-4 gap-2">
          ...
          {selectedType === "compose_generation" && <div className="grid grid-cols-2 gap-2">
            <label className="grid gap-1"><span className="text-slate-400">单镜头时长</span><input type="number" step="0.5" className="rounded-md border border-white/10 bg-white/5 px-3 py-2 outline-none" onFocus={rememberSelectedNodeEdit} value={String(selectedData.duration_per_shot || "4")} onChange={(event) => updateSelectedData("duration_per_shot", event.target.value)} /></label>
            <label className="grid gap-1"><span className="text-slate-400">转场方式</span><select className="rounded-md border border-white/10 bg-slate-900 px-3 py-2 outline-none" onFocus={rememberSelectedNodeEdit} value={String(selectedData.transition || "cut")} onChange={(event) => updateSelectedData("transition", event.target.value)}>
              <option value="cut">硬切</option>
              <option value="fade">淡入淡出</option>
              <option value="slide">滑动</option>
            </select></label>
            <label className="grid gap-1"><span className="text-slate-400">字幕样式</span><input className="rounded-md border border-white/10 bg-white/5 px-3 py-2 outline-none" placeholder="例如底部黄字黑描边" onFocus={rememberSelectedNodeEdit} value={String(selectedData.subtitle_style || "底部白字黑描边")} onChange={(event) => updateSelectedData("subtitle_style", event.target.value)} /></label>
            <label className="grid gap-1"><span className="text-slate-400">合成音色</span><select className="rounded-md border border-white/10 bg-slate-900 px-3 py-2 outline-none" onFocus={rememberSelectedNodeEdit} value={String(selectedData.voice || "zh-CN-XiaoxiaoNeural")} onChange={(event) => updateSelectedData("voice", event.target.value)}>
              <option value="zh-CN-XiaoxiaoNeural">晓晓</option>
              <option value="zh-CN-YunxiNeural">云希</option>
              <option value="zh-CN-XiaoyiNeural">晓伊</option>
            </select></label>
          </div>}
          {selectedType === "compose_generation" && <label className="grid gap-1"><span className="text-slate-400">BGM URL</span><input className="rounded-md border border-white/10 bg-white/5 px-3 py-2 outline-none" placeholder="填写背景音乐素材 URL，可留空" onFocus={rememberSelectedNodeEdit} value={String(selectedData.bgm_url || "")} onChange={(event) => updateSelectedData("bgm_url", event.target.value)} /></label>}
      ...
      {showTasks && <aside className="absolute bottom-6 left-[500px] z-20 max-h-[320px] w-[380px] overflow-auto rounded-lg border border-white/10 bg-slate-950/90 p-4 shadow-2xl backdrop-blur">
        ...
        <div className="mt-3 grid grid-cols-5 gap-2 text-xs">
          <button disabled={busy || !filteredTasks.length} className="rounded-md border border-white/10 px-3 py-2 text-slate-200 hover:bg-white/10 disabled:opacity-50" onClick={() => void batchFilteredTaskAction("submit")}>批量提交</button>
          ...
          <button disabled={busy || !filteredTasks.some((task) => task.status === "pending" || task.status === "running")} className="rounded-md border border-red-400/30 px-3 py-2 text-red-100 hover:bg-red-500/10 disabled:opacity-50" onClick={() => void batchFilteredTaskAction("cancel")}>取消运行</button>
