...
- 第 3 阶段：创作工作台。完成脚本拆解、角色设定、分镜表格、单镜头重试、批量生成、素材预览、时间线编排、成片合成和导出。已追加完成 LibTV 风格全画幅节点画布，支持平台业务节点编排、节点大纲搜索、问题筛选和批量选中、项目分镜面板、分镜状态筛选、分镜关键词搜索、分镜排序、分镜多选、未铺设分镜一键选择、分镜铺设状态识别与筛选、已铺设分镜链路定位、按单个、选中或筛选后的全部分镜添加生成链路、工作流预设、我的工作流预设、搜索添加、最近使用节点、快速添加上游/下游节点并自动连线、节点面板拖拽添加、节点语义输入/输出连接桩、节点右键快捷操作、连线右键快捷操作、选区批量操作、选区批量命名、连线标签编辑、复制连线 ID/参数 JSON/定位链接、连线颜色标记、连线样式、连线端口映射、连线插入节点、连线反转方向、连线启用/禁用、选区运行、选区一键串联、选区左/右/水平居中/顶部/底部/垂直居中对齐与等距分布、选区打组/选中同组节点/分组名称编辑/取消分组、选区内部连线批量启用/禁用、颜色标记、样式和删除、空白处双击添加、素材拖入、画布内素材预览、素材类型筛选、素材关键词搜索、任务状态筛选、任务关键词搜索、上游输入预览和参数填充、分镜绑定、参数编辑、节点颜色标记、节点折叠/展开、节点参数编辑可撤销、生成参数预设、连线补全、节点备注、节点类型切换、节点锁定/解锁、节点启用/禁用、节点复制、节点/选区断开连线、选区复制/剪切/粘贴到当前视图中心、选区保存为预设、链路复制粘贴到当前视图中心、工作流 JSON 文件导入导出、工作流剪贴板导入、选区 JSON 文件导出、视口保存/恢复、视图缩放、视图快速导航、画布视图书签、画布导航器开关、拖拽框选模式开关、网格吸附、方向键微调节点位置、节点/选区置顶置底、全选/反选/清空选区、上游/下游链路选择、同类型/同状态/同标记/禁用/孤立/起点/终点/错误/提醒/运行中任务/失败任务节点选择、撤销/重做、单节点运行、链路运行、全图运行、自动整理、选区整理、画布自检、运行诊断、任务操作和保存。
